...
_CACHE_KEY = "_youtube_analyzer_models_cache"

if not TYPE_CHECKING and hasattr(sys, _CACHE_KEY):
    _cached = getattr(sys, _CACHE_KEY)
    PlatformEnum = _cached["PlatformEnum"]
    IntentEnum = _cached["IntentEnum"]
    utc_now = _cached["utc_now"]
    Topic = _cached["Topic"]
    Keyword = _cached["Keyword"]
    Query = _cached["Query"]
    IntentCluster = _cached["IntentCluster"]
    Competitor = _cached["Competitor"]
    Citation = _cached["Citation"]
    ContentOpportunity = _cached["ContentOpportunity"]
else:
    ...
    class Topic(SQLModel, table=True):
        ...
        canonical_id: str = Field(
            index=True, unique=True, description="Human-readable ID, e.g. GA-001"
        )
        name: str = Field(
            index=True, description="Canonical topic name, e.g. 'Google Ads untuk UMKM'"
        )
